mall.controller('bonusListController', function ($rootScope, $scope, $http, $state,$stateParams) {
    $scope.options = {};
    ...
    $http.get('/bonus/getAll', {params: {pageNum: $scope.page, pageSize: 20}}).then(function (response) {
        $scope.options.paginate = response.data.data;
    }, function (error) {
    });

    $scope.itemClick = function (id) {
        $state.go('bonusDetail', {id: id});
    };

    $scope.itemOnline = function (id) {
        $http.put('/bonus/offline', {
            id: id,
            online: 1
        },{
